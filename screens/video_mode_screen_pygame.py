...
def play_youtube_with_mpv(url):
    try:
        player = mpv.MPV(fullscreen=True, loop="inf", input_default_bindings=True, input_vo_keyboard=True)
        player.play(url)
...
class VideoModeScreenPygame:
    ...
    def update_video_widget(self):
        ...
        if is_youtube_url(random_video_path):
            play_youtube_with_mpv(random_video_path)
            return
